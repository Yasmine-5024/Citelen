...
function RecencyTimeline({
  ...
  const maxYear = 2025;
  const recentStart = maxYear - 5; // 2020
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
        ...
        <span style={{
          ...
          color: recencyScore >= 50 ? C.green : recencyScore >= 25 ? C.yellow : C.orange,
        }}>
      ...
      <div style={{ position: "relative", height: 26, marginBottom: 20 }}>
        ...
        <div style={{
          ...
          height: 10, background: `${C.accent}18`,
          borderRadius: 6,
        }} />
      ...
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: C.textDim }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
            <span style={{ display: "inline-block", width: 8, height: 3, background: C.blue, borderRadius: 2 }} />
            Median year
          </span>
          <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
            <span style={{ display: "inline-block", width: 8, height: 8, background: `${C.green}35`, borderRadius: 2, border: `1px solid ${C.green}60` }} />
            Last 5 years
          </span>
        </div>
        <span>{maxYear}</span>
        ...
        {venues.map((v, i) => {
          ...
          return (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <span style={{ fontSize: 11, color, fontWeight: 700, width: 24, textAlign: "right", flexShrink: 0 }}>
                {v.count}
              </span>
              <span style={{ fontSize: 10, color: C.textDim, width: 28, textAlign: "right", flexShrink: 0 }}>
                {pct}%
              </span>
            </div>
          );
    ...
    <div style={{
      ...
      {showConcentration && (
        <div style={{
          ...
          <span style={{ color: C.yellow }}>⚠ Concentrated sourcing</span> —{" "}
          <span style={{ color: C.text }}>{authors[0].name}</span> accounts for{" "}
          <span style={{ color: C.yellow, fontWeight: 700 }}>{topPct}%</span> of all citations.
        </div>
      )}
        ...
        {authors.map((author, i) => {
          ...
          return (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <span style={{
                width: 160, flexShrink: 0, overflow: "hidden",
...
function YearChart({
  ...
  const decades  = [
    ...
    { label: "2020s",    count: twenties, color: C.green    },
  ].filter(d => d.count > 0);
    ...
    <div style={{
      ...
      <div style={{
        ...
        {distribution.map((bucket, i) => {
          ...
          const barColor   = isSelected ? C.green : isRecent ? C.accent : `${C.accent}70`;
          ...
          return (
            ...
            >
              {bucket.count > 0 && (
                <span style={{
                  fontSize: 9, color: isSelected ? C.green : C.textDim,
              ...
              <div style={{
                ...
                boxShadow: isSelected ? `0 0 8px ${C.green}60` : "none",
              }} />
              <span style={{
                position: "absolute", bottom: 0,
                fontSize: 8.5,
                color: isSelected ? C.green : C.textDim,
                fontWeight: isSelected ? 700 : 400,
                whiteSpace: "nowrap",
          ...
          {drillCitations.map((cit, i) => {
            ...
            return (
              <a
                ...
                onMouseEnter={e => (e.currentTarget.style.background = `${C.green}14`)}
                onMouseLeave={e => (e.currentTarget.style.background = `${C.green}08`)}
              >
                <span style={{ fontSize: 11, fontWeight: 700, color: C.green, fontFamily: "monospace", flexShrink: 0 }}>
                  {cit.id}
                </span>
                <span style={{ fontSize: 11, color: C.textMuted, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 }}>
                  {cit.title}
                </span>
                <span style={{ fontSize: 10, color: C.green, flexShrink: 0, opacity: 0.7 }}>↗</span>
              </a>
            );
          })}
        </div>
  ...
  return (
    <div style={{ padding: "16px 16px 24px" }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
        <StatBox
          value={data.median_year ?? "—"}
          label="Median Year"
          sub="Publication age"
          color={C.blue}
        />
        <StatBox
          value={`${data.recency_score}%`}
          label="Recency"
          sub="Last 5 years"
          color={data.recency_score >= 50 ? C.green : data.recency_score >= 25 ? C.yellow : C.orange}
        />
        <StatBox
